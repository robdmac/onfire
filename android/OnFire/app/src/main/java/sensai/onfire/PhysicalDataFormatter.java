...
class PhysicalDataFormatter {
    ...
    private OnFireApplication onFireApplication = OnFireApplication.getInstance();
        ...
        switch (Format) {
            ...
            case FORMAT_ACCURACY:   // Accuracy
                switch (onFireApplication.getPrefUM()) {
                    ...
                    case UM_METRIC_MS:
                        _PhysicalData.Value = String.valueOf(Math.round(Number));
                        _PhysicalData.UM = onFireApplication.getString(R.string.UM_m);
                        return(_PhysicalData);
                    case UM_IMPERIAL_MPH:
                    case UM_IMPERIAL_FPS:
                    case UM_NAUTICAL_MPH:
                    case UM_NAUTICAL_KN:
                        _PhysicalData.Value = String.valueOf(Math.round(Number * M_TO_FT));
                        _PhysicalData.UM = onFireApplication.getString(R.string.UM_ft);
                        return(_PhysicalData);
                }

            case FORMAT_BEARING:    // Bearing (Direction)
                switch (onFireApplication.getPrefShowDirections()) {
                    case 0:         // NSWE
                        final String N = onFireApplication.getString(R.string.north);
                        final String S = onFireApplication.getString(R.string.south);
                        final String W = onFireApplication.getString(R.string.west);
                        final String E = onFireApplication.getString(R.string.east);
                ...
                switch (onFireApplication.getPrefUM()) {
                    ...
                    case UM_METRIC_MS:
                        if (Number < 1000) {
                            ...
                            _PhysicalData.UM = onFireApplication.getString(R.string.UM_km);
                        }
                        return(_PhysicalData);
                    case UM_IMPERIAL_MPH:
                    case UM_IMPERIAL_FPS:
                        if ((Number * M_TO_FT) < 1000) {
                            ...
                            _PhysicalData.UM = onFireApplication.getString(R.string.UM_mi);
                        }
                        return(_PhysicalData);
                    case UM_NAUTICAL_KN:
                    case UM_NAUTICAL_MPH:
                        ...
                        _PhysicalData.UM = onFireApplication.getString(R.string.UM_nm);
                        return(_PhysicalData);
                }
        }
        return(_PhysicalData);
    ...
    public PhysicalData format(double Number, byte Format) {
        ...
        switch (Format) {
            case FORMAT_LATITUDE:   // Latitude
                _PhysicalData.Value = onFireApplication.getPrefShowDecimalCoordinates() ?
                    String.format("%.9f", Math.abs(Number)) : Location.convert(Math.abs(Number), Location.FORMAT_SECONDS);
                _PhysicalData.UM = Number >= 0 ? onFireApplication.getString(R.string.north) : onFireApplication.getString(R.string.south);
                return(_PhysicalData);
            ...
            case FORMAT_LONGITUDE:  // Longitude
                ...
                _PhysicalData.UM = Number >= 0 ?
                    onFireApplication.getString(R.string.east) : onFireApplication.getString(R.string.west);
                return(_PhysicalData);
