...
class Exporter extends Thread {
    ...
    public Exporter(long ID, boolean ExportKML, boolean ExportGPX, boolean ExportTXT, String SaveIntoFolder) {
        track = OnFireApplication.getInstance().GPSDataBase.getTrack(ID);
        AltitudeManualCorrection = OnFireApplication.getInstance().getPrefAltitudeCorrection();
        EGMAltitudeCorrection = OnFireApplication.getInstance().getPrefEGM96AltitudeCorrection();
        getPrefKMLAltitudeMode = OnFireApplication.getInstance().getPrefKMLAltitudeMode();
        ...
        try {
            ...
            if (track.getNumberOfPlacemarks() > 0) {
                ...
                for (int i = 0; i <= track.getNumberOfPlacemarks(); i += GroupOfLocations) {
                    ...
                    placemarkList.addAll(OnFireApplication.getInstance().GPSDataBase.getPlacemarksList(track.getId(), i, i + GroupOfLocations - 1));
    ...
    private class AsyncGeopointsLoader extends Thread {
        ...
        public void run() {
            ...
            for (int i = 0; i <= track.getNumberOfLocations(); i += GroupOfLocations) {
                //Log.w("myApp", "[#] Exporter.java - " + (i + GroupOfLocations));
                lList.addAll(OnFireApplication.getInstance().GPSDataBase.getLocationsList(track.getId(), i, i + GroupOfLocations - 1));
                if (!lList.isEmpty()) {
